...
class ImageDehazeDataset(data.Dataset):
    """Define dataset."""

    def __init__(self, root, transforms=get_transform()):
        """Init dataset."""
        super(ImageDehazeDataset, self).__init__()
...
def train_data(bs):
    ...
    train_ds = ImageDehazeDataset(train_dataset_rootdir, get_transform(train=True))
    print(train_ds)
...
def test_data(bs):
    ...
    test_ds = ImageDehazeDataset(test_dataset_rootdir, get_transform(train=False))
...
def ImageDehazeDatasetTest():
    """Test dataset ..."""

    ds = ImageDehazeDataset(train_dataset_rootdir)
    print(ds)
    # src, tgt = ds[10]
